...
def test_change_password_success(userDB):
    ...
    mock_session.scalar.return_value = userDB
    user_repo = UserRepository(mock_session)
    new_pass = 'new_secure_hash_123'
    user_repo.change_password(user_id=userDB.id, new_hashed_password=new_pass)
    assert userDB.hashed_password == new_pass
...
def test_change_password_user_not_found():
    ...
    with pytest.raises(UserNotFoundError) as exc_info:
        user_repo.change_password(
            user_id=999, new_hashed_password='any_password'
        )
